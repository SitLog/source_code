...
[
	...
	[
		 ...
		 arcs ==> [
				     success : [
					             say('I have fetched the objects now i will carry them to the designated locations')
	...
	[
		 ...
		 type ==> neutral,      
		 arcs ==> [	
				     empty : empty => carry(Orders, ObjectList, FetchedObjects, apply(find_destination(L,O), [get(left_arm,LeftArm),Orders]), apply(find_destination(R,O), [get(right_arm,RightArm),Orders]))
			       ]     
	],	
