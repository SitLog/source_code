...
[% situations for following a person and registering points and products
	...
	[ % try to see the gesture
		id ==> wait_gesture,
		type ==> recursive,
		embedded_dm ==> see_gesture(waving, nearest, 3, GestureParameters, GestureStatus),
		arcs ==> [
			success : empty => get_position(waving, GestureParameters),
			error : empty => get_position(not_detected,_)
		]
	],

	[ % register location based on the pointing gesture
		id ==> get_position(waving, [person(ID,R,O,P)|_]),
		type ==> neutral,
		arcs ==> [
			empty : [
