...
[% list of situations
	...
	[% initial situation
		id ==> ask_first,
		type ==> recursive,
		embedded_dm ==> ask_restaurant_commands('What would you like to drink.',drink,false,2,OrderTaken,AskStatus),
		arcs ==> [
			success : [say(['I heard',OrderTaken])] => form_order(drink(OrderTaken)),
			error : [
	...
	[% asking next order
		id ==> ask_next(true),
		type ==> recursive,
		embedded_dm ==> ask_restaurant_commands('Which is your next order',restaurant_command,false,2,OrderTaken,AskStatus),
		arcs ==> [
	...
	[% check drink
		id ==> check_drink(drink(Drink)),
		type ==> neutral,
		arcs ==> [
			empty : [
				apply(is_drink_in_list(A,D,R),[available_drinks,Drink,Res])
			] => decide(Res,Drink)
		]
	],
	[% check drink
		id ==> decide(false,Drink),
		type ==> neutral,
		arcs ==> [
			empty : [
				say(['We do not have ',Drink])
			] => ask_first
		]
	],
