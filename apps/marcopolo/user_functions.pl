...
list_parser([],[]).
list_parser([Name:List_Value| Tail], Out):-
        list_parser(Tail, Tmp),
        append(List_Value, Tmp).
...
%% Update KB given that user asked for a comestible
upd_kb_user_asked(_,no_class) :-
   assign_func_value(empty).
      
upd_kb_user_asked(no_object,_) :-
   open_kb(KB),
   object_property_value(user,asked_comestible,KB,Val),
   aux_upd_kb_user_asked(Val,KB,NewKB),
   save_kb(NewKB),
   assign_func_value(empty).
      
upd_kb_user_asked(Obj,Class) :-
   open_kb(KB),
   object_property_value(user,asked_comestible,KB,Val),
   aux_upd_kb_user_asked(Val,KB,Tmp_KB),
   add_class_property(Class,asked,Obj,Tmp_KB,NewKB),
   save_kb(NewKB),
   assign_func_value(empty).  
   
aux_upd_kb_user_asked(unknown,KB,KB1):-
   add_object_property(user,asked_comestible,yes,KB,KB1).

aux_upd_kb_user_asked(_,KB,KB).
...
consult_kb_preferences(Object,Class,Pref_Name):-
   open_kb(KB),
   (Object==no_object ->
      (Class==no_class ->
         PrefList = []
...
get_obj_class(Element):-
    open_kb(KB),
    there_is_object(Element,KB,Val1),
    (Val1==yes ->
         class_of_an_object(Element, KB, Class),
         assign_func_value([Element,Class])         
     | otherwise ->
         there_is_class(Element,KB,Val2),
         (Val2==yes ->
             assign_func_value([no_object,Element])
          | otherwise ->
             assign_func_value([no_object,no_class])
         )
     ).
...
get_head(L):-
    is_list(L),
    open_kb(KB),
    aux_get_head(L,KB,Obj),
    assign_func_value(Obj).
    
get_head(_):-
    assign_func_value(non_list).

aux_get_head([], _, empty_list).
    
aux_get_head([object(Object_ID, X, Y, Z, O1, O2, O3, O4, Conf)|_], KB, object(Object_ID, X, Y, Z, O1, O2, O3, O4, Conf)):-
    object_property_value(Object_ID, object_mode, KB, moped).
    
aux_get_head([_|Tail], KB, Obj):-
   aux_get_head(Tail, KB, Obj).
...
%%% Author: Noe Hdez
%%% This functions, with the aid of aux_fun, add new elements in the list 'New' to the ones present is list 'Old'
remove_repeated(New, Old_Cat, Scan, Tilt, RobotHeight):-
    print('Possibly adding new seen objects'), nl,
    print('height: '), print(RobotHeight), nl,
    print('tilt: '), print(Tilt), nl,
...
.
aux_func([object(Name,X,Y,Z,Q1,Q2,Q3,Q4,Score)|More_New], Old_Cat, Scan, Tilt, RobotHeight, R1):-
    open_kb(KB),
    class_of_an_object(Name, KB, Category),
    nl,print(Category),nl,
    is_new(Category, Old_Cat),
    min_robot_height(MinH),
    max_robot_height(MaxH),
    nl, nl, print('* * * * * * * * * * * * * * * * * * * * *'), print('Calculating height'), print('* * * * * * * * * * * * * * * * * * * * *'), nl, nl,
    cam2arm(1, RobotHeight, [X,Y,Z], [Tilt,0.04,-0.06], [-0.05,0.22,0.2248], [Height, Angle, Distance]),
    MinH =< Height, Height =< MaxH,     
    nl, print('Object with proper height!! Yay Yay  '), print(Height), nl,
...
manana :-
    ...
    assign_func_value(Tomorrow_Week)
.
