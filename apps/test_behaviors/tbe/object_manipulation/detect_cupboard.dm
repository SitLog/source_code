...
[
	 ...
	 type ==> neutral,
	 arcs ==> [
                  empty : [get(pos,Pos)] => identify_objects([],_,Pos)
	          ]
     ],
	 ...
	 out_arg ==> [Cat],
	 arcs ==> [
		  empty : [get(categorias,Cat)] => apply( when(I_,T_,F_),[Cat==[],error,success] )
                  ]
     ],
     ...
     [
         id ==> identify_objects([], _, [Pt|More_Pts]),
         type ==> recursive,
         ...
         arcs ==> [
                  success : [
                             ...
                             get(scan_st,Scan),
                             get(tilt_st,Tilt)                           
                            ] => identify_objects(Scan,Tilt,More_Pts),
	          error   : empty => error
                  ]
     ],
	 ...
	 arcs ==> [
		  empty : [get(last_scan, SVal),
                           get(tilt_st,Tilt)] => identify_objects( apply( list_fil(S_,V_),[Scan,SVal] ),Tilt,Pts)
                  ]    
     ],     
	 ...
	 type ==> recursive,
         embedded_dm ==> scan(object, X, HFull, VFull, object, Found_Obj, false, false, Status),
	 arcs ==> [
	          success :
	     	      ...
	     	      => set_object_list(HFull,VFull,Pts, apply(   remove_repeated(V_, W_, X_, Y_, Z_), [Found_Obj, Current_Cate, LastSc, LastTi, LastHeight]   )   ),
	 ...
	 arcs ==> [
			    ...
			    => identify_objects( apply( list_hor(H_,V_), [HFull,HVal] ),  apply( list_fil(M_,N_), [VFull,VVal] ), Pts )
                  ]
     ],
     ...
     type ==> final,
     in_arg ==> [Final_List],
     diag_mod ==> detect_cupboard(_, _, _, Final_List)
     ],
     ...
     id ==> error,
     type ==> final,
     diag_mod ==> detect_cupboard(_, _, _, [])
     ]   
