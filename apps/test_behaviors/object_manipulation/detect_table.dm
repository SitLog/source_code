%%
diag_mod(detect_table(Table_Mode),
[
     ...
     type ==> recursive,
     embedded_dm ==> find(plane,X,Pts,Scan,Tilt,Table_Mode,FndObjs,_,false,false,false,_),
     arcs ==> [
...
[
     scan_st ==> [-25.0,25.0],                         %% <--- Attention!!
     tilt_st ==> [-20.0],                                  %% <--- Attention!!
     tbl_pts ==> [turn_precise=>(90.0),turn_precise=>(90.0),turn_precise=>(90.0)]  %% <--- Attention!!
