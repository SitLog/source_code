...
[
  ...
  arcs ==> [
           empty:[
                  ...
                  say(['So, I am going to visit the preferred locations, where ',Item,' can be found'])
  ...
  type ==> neutral,
  arcs ==> [
           empty : [say(['No more locations to find the ',ObjName])] => error
           ]
  ],
  ...
  arcs ==> [
            success:[
                     assign(Current_Loc, apply( get_current_position(L_,R_),[Locations,Remaining_Locs] )),
                     apply( update_locations(F_,CL_),[Found_objs,Current_Loc] ),
                     assign(Obj_to_take,apply( object_present_scene(Ob_,FO_),[ObjName,Found_objs] )),
		     apply(consult_kb_value_object_property(X1_,X2_,X3_),[Current_Loc,name,Loc_Name]),
		     apply( when(Si_,Verdadero_,Falso_),
			          [Obj_to_take==not_found,
			           say(['The', ObjName ,'is not found in', Loc_Name]),
				   empty
				  ])                                
  ...
  %% If successful, we get another item now with left hand
  ...
  arcs ==> [
           success:[get(more_items,Items),inc(arm_id,_),
  ...
  arcs ==> [
           success:[apply( reset_obj_loc(O_),[Obj_Name] )] => almost_success,
           error  :[robotheight(1.20),
                    set(last_height,1.20),
                    advance_fine(-0.15, _)
                   ] => see_again(Obj_Name)
  ...
  arcs ==> [
            success:[ get(arm_id,Arm) ] => get_obj(Obj_to_take,Obj_Name,Arm) , 
            error  : empty => error
           ]
  ],
  ...
  out_arg ==> [Item_Loc],
  arcs ==> [
           empty:[get(item_location,Item_Loc)] => success
           ]
  ],
