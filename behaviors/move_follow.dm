...
diag_mod(move_follow(Movements,Status),
[
	...
	[
		id ==> verify_case(coordinate,ListOfMovements),
      		type ==> neutral,
      		arcs ==> [
			 empty : [switcharm(0),robotheight(1.35),set(last_height,1.35),switchpose('nav')] => go_to_point(ListOfMovements)
      			]
    	],	
	...
	[
		id ==> verify_case(label,ListOfLabels),
			 ...
			 empty : [switcharm(0),robotheight(1.35),set(last_height,1.35),switchpose('nav')] => go_to_point( apply(from_labels_to_coordinates(L),[ListOfLabels]) )
      			]
    	],	
	...
	%Final situations  
    	...
    	[
      		id ==> success,
      		type ==> final,
		diag_mod ==> move_follow(_, ok)
    	],

    	[
      		id ==> error,
      		type ==> final,
		diag_mod ==> move_follow(_, navigation_error)
    	]
