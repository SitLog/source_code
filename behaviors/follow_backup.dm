...
diag_mod(follow(Mode,Expected_Event,Happened_Event,Status), 
		...
		[
      			...
      			type ==> neutral,
      			arcs ==> [
        				empty : [apply(initialize_KB,[]),robotheight(1.40),set(last_height,1.40),tiltv(-10.0),set(last_tilt,-10.0),say('Stand in front of me six feet away, waving your hands')] => detect_gesture
		...
		[  
			...
			embedded_dm ==> see_gesture(waving, nearest, 3, Gesture_Value, StatusGesture),
      			arcs ==> [
        			success : say('Perfect. Turn your back on me, please.') => detect,
    ...
    [  
      		id ==> memorize,
      		type ==> recursive,
		    embedded_dm ==> memorize_body(nearest,1,michael,Body_Positions,Status),
      		arcs ==> [
        			success : [say('I memorized you'),switchpose('nav'),avanzar(1.00),say('Perfect, I am following you now.'),say('Wave one hand if you want me to stop.')] => start(continue),%follow_until_event(Expected_Event),
        			error : [say('I did not memorize the person')] => apply(when(If,TrueVal,FalseVal),[C>5,error,detect_gesture])			
      			]
    ],
		...
		[  
			...
			embedded_dm ==> see_gesture(hand_up, nearest, 3, Gesture_Value, StatusGesture),
      			arcs ==> [
        			...
        			error : empty =>  follow_until_event(gesture_only)
