%
%
% not_found error in the take behavior for the GPSR task
% Author: Noe Hernandez   
%
%
diag_mod(take_not_found(object(Name, _, _, _, _, _, _, _, _), Arm, Remaining_tasks, Tasks),
[
    % Initial situation
    [
    id ==> is,
    type ==> neutral,
    arcs ==> [
             empty : [inc(tk_not_fnd_error,Error_Number)] => apply( when(If, Verdadero, Falso), [Error_Number<2, recovery_protocol, error] )
             ]
    ],
    ...
    type ==> neutral,
    arcs ==> [
             empty : [say(['I did not find the ', Name]),
    ...
    type ==> final,
    in_arg ==> [Tasks],
    diag_mod ==> take_not_found(_, _, _, Tasks)
    ],
