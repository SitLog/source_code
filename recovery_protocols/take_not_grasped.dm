%
%
% not_grasped error in the take behavior for the GPSR task
% Author: Noe Hernandez   
%
%
diag_mod(take_not_grasped(object(Name, _, _, _, _, _, _, _, _), Arm, Remaining_tasks, Tasks),
[
    % Initial situation
    [
    id ==> is,
    type ==> neutral,
    arcs ==> [
             empty : [inc(tk_not_grasped_error,Error_Number)] => apply( when(If, Verdadero, Falso), [Error_Number<2, recovery_protocol, error] )
             ]
    ],
    ...
    type ==> neutral,
    arcs ==> [
             empty : [say(['I did not grasp the ', Name]),
    ...
    type ==> final,
    in_arg ==> [Tasks],
    diag_mod ==> take_not_grasped(_, _, _, Tasks)
    ],
